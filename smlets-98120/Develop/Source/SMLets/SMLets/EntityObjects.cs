...
public class FilterCmdletBase : ObjectCmdletHelper
    {
    public FilterCmdletBase()
    {
        ObjectRetrievalOptions = ObjectRetrievalOptions.NonBuffered;
        PageSize = 250;
    }
    ...
    protected string _filter = null;
        ...
        [Parameter]
        public ObjectRetrievalOptions ObjectRetrievalOptions { get; set; } 
...
public class GetSMObjectCommand : FilterCmdletBase
{
    ...
    protected override void BeginProcessing()
    {
        ...
        {
            if (!base.MyInvocation.BoundParameters.ContainsKey("QueryOption"))
            {
                QueryOption = new ObjectQueryOptions();
                ...
                if (MaxCount != Int32.MaxValue)
                {
                    ...
                    QueryOption.ObjectRetrievalMode = ObjectRetrievalOptions;
                }
            }
        }
        string sortProperty = SortBy;

        if (!base.MyInvocation.BoundParameters.ContainsKey("ClassName"))
        {
            Regex r = new Regex(ClassName, RegexOptions.IgnoreCase);
            Class = _mg.EntityTypes.GetClasses().FirstOrDefault(m => r.Match(m.Name).Success);
            if (Class == null)
            {
                throw new ArgumentException(string.Format("Class {0} was not found.", ClassName), "ClassName");
            }
        }
    }
    ...
    protected override void ProcessRecord()
    {
        ...
        else // OK, we got a criteria - we'll use that
        {
            ...
            var reader = _mg.EntityObjects.GetObjectReader<EnterpriseManagementObject>(Criteria, QueryOption);
            reader.PageSize = PageSize;
            foreach (EnterpriseManagementObject o in reader)
            {
                count++;
                if (NoAdapt)
                {
...
public class GetSCSMRelationshipObjectCommand : ObjectCmdletHelper
{
    ...
    public Guid Id
    {
        ...
        [Parameter(ParameterSetName = "RELATIONSHIPNAME", Mandatory = true, ValueFromPipeline = true, Position = 0)]
    ...
    private enum QueryBy { TargetClass, SourceClass, Target, Source, Relationship, TargetAndRelationship, RelationshipName };

    #region GetRelationshipsHelper
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(ManagementPackRelationship r)                             { return GetRelationshipObjects(null, null, r, QueryBy.Relationship, null); }
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(ManagementPackRelationship r, string filter)              { return GetRelationshipObjects(null, null, r, QueryBy.Relationship, filter); }
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(string filter)                                            { return GetRelationshipObjects(null, null, null, QueryBy.Relationship, filter); }
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(ManagementPackClass c, QueryBy q)                         { return GetRelationshipObjects(c, null, null, q, null); }
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(ManagementPackClass c, QueryBy q, string filter)          { return GetRelationshipObjects(c, null, null, q, filter); }
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(EnterpriseManagementObject emo, QueryBy q)                { return GetRelationshipObjects(null, emo, null, q, null); }
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(EnterpriseManagementObject emo, QueryBy q, string filter) { return GetRelationshipObjects(null, emo, null, q, filter); }
    private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(EnterpriseManagementObject emo, ManagementPackRelationship r) { return GetRelationshipObjects(null, emo, r, QueryBy.TargetAndRelationship, null); }
        

        private IEnumerable<EnterpriseManagementRelationshipObject<EnterpriseManagementObject>> GetRelationshipObjects(ManagementPackClass classType, EnterpriseManagementObject emo, ManagementPackRelationship r, QueryBy q, string filter)
    {
        ...
        try
        {
            if (QueryOption == null)
            {
                QueryOption = ObjectQueryOptions.Default;
            }
                ...
                case QueryBy.TargetClass:
                    {
                        if (criteria != null)
                        {
                            Results = _mg.EntityObjects.GetRelationshipObjectsByTargetClass<EnterpriseManagementObject>(criteria, classType, QueryOption);
                        }
                        else
                        {
                            Results = _mg.EntityObjects.GetRelationshipObjectsByTargetClass<EnterpriseManagementObject>(classType, QueryOption);
                        }
                        break;
                    }
                case QueryBy.SourceClass:
                    {
                        if (criteria != null)
                        {
                            Results = _mg.EntityObjects.GetRelationshipObjectsBySourceClass<EnterpriseManagementObject>(criteria, classType, QueryOption);
                        }
                        else
                        {
                            Results = _mg.EntityObjects.GetRelationshipObjectsBySourceClass<EnterpriseManagementObject>(classType, QueryOption);
                        }
                        break;
                    }
                case QueryBy.Target:
                    {
                        Results = _mg.EntityObjects.GetRelationshipObjectsWhereTarget<EnterpriseManagementObject>(emo.Id, QueryOption);
                        break;
                    }
                case QueryBy.TargetAndRelationship:
                    {
                        Results = _mg.EntityObjects.GetRelationshipObjectsWhereTarget<EnterpriseManagementObject>(emo.Id, r, DerivedClassTraversalDepth.Recursive, this.Recursive ? TraversalDepth.Recursive : TraversalDepth.OneLevel, QueryOption);
                        break;
                    }
                case QueryBy.Source:
                    {
                        if (criteria != null)
                        {
                            Results = _mg.EntityObjects.GetRelationshipObjectsWhereSource<EnterpriseManagementObject>(emo.Id, criteria, this.Recursive ? TraversalDepth.Recursive : TraversalDepth.OneLevel, QueryOption);
                        }
                        else
                        {
                            Results = _mg.EntityObjects.GetRelationshipObjectsWhereSource<EnterpriseManagementObject>(emo.Id, this.Recursive ? TraversalDepth.Recursive : TraversalDepth.OneLevel, QueryOption);
                        }
                        break;
                    }
                case QueryBy.Relationship:
                    {
                        if (criteria != null)
                        {
                            ...
                            Results = _mg.EntityObjects.GetRelationshipObjects<EnterpriseManagementObject>(criteria, QueryOption);
                        }
                        else
                        {
                            Results = _mg.EntityObjects.GetRelationshipObjects<EnterpriseManagementObject>(r, DerivedClassTraversalDepth.Recursive, QueryOption);
    ...
    protected override void ProcessRecord()
    {
        ...
        else if (this.ParameterSetName == "RELATIONSHIPNAME")
        {
            Regex r = new Regex(RelationshipName, RegexOptions.IgnoreCase);
            foreach (ManagementPackRelationship o in _mg.EntityTypes.GetRelationshipClasses())
            {
                if (r.Match(o.Name).Success)
                {
                    WriteObject(GetRelationshipObjects(o, Filter), true);
                }
            }
        }
